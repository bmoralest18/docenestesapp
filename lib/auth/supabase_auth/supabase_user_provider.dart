...
class EvaluaMeSupabaseUser extends BaseAuthUser {
  EvaluaMeSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> evaluaMeSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EvaluaMeSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
